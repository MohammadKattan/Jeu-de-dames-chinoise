...
let gagnant _ = Libre
(*Q2*)let est_dans_losange = fun ((i,j,k):case)->
...
let rec tourne_case_coul liste_case_coul = match liste_case_coul with 
    []-> []
  | t::q-> (match t with 
      |(k,coul)->((tourner_case 1 k),coul)::(tourne_case_coul q))
...
tourne_config (([(-6,3,3),Vert;(-5, 3, 2),Vert;(-5, 2, 3),Vert]),([Vert;Jaune;Rouge;Noir;Bleu]));; 
(*Q7*)
...
(* sont_voisines (6,-3,-3) (5,-2,-3);; *)
(*sont_voisines (5, -2, -3) (6, -2, -2);;*)
(*Q8  --- a corriger *) 

let rec case_dans_config ((c):case) (conf:configuration) = let (i,j,k)= c in 
  if est_dans_etoile c =false then false
  else  match conf with
      (case_co_list,color_list)->  List.mem_assoc (i,j,k) case_co_list;;

case_dans_config (5,-3,-2) (([(5,-3,-2),Bleu;(5,-2,-3),Rouge]),([Bleu;Jaune;Rouge]));;
...
(* quelle_couleur (6,-3,-3) (([(5,-3,-2),Bleu;(5,-2,-3),Rouge]),([Bleu;Jaune;Rouge]));; *)
(* quelle_couleur (5,-2,-3) (([(5,-3,-3),Bleu;(5,-2,-3),Rouge]),([Bleu;Jaune;Rouge]));; *)

(*quelle_couleur (7,-3,-2) (([(6,-3,-3),Bleu;(5,-2,-3),Rouge]),([Bleu;Jaune;Rouge]));;*)
...
(*remplir_triangle (([(-6,6,3),Bleu;(5,-2,-3),Rouge]),([Bleu;Jaune;Rouge])) Vert (-6,3,3);;
  remplir_triangle (([]),([Bleu;Jaune;Rouge])) Vert (-6,3,3);;*)
...
(* est_dep_unit ([],([Vert;Bleu;Jaune;Rouge]))  (-4,3,1) (-3,3,0);; *)
(* 13 *)
(* premiere méthode  *)
...
(* fait_dep_unit ([(6,-3,-3),Rouge;(-4,3,1),Vert],([Vert;Bleu;Jaune;Rouge]))  (-4,3,1) (-3,3,0);; *)
(* 14 *)
...
(* let mis_a_jour_configuration config cp = 
   match cp with 
   | (Sm([]))-> Error "choisir deux cases"
   |(Sm([c1]))->Error "choisir deux cases"
   |(Sm(c1::c2::q))->if est_dep_unit config c1 c2=false  then Error "ce n'est pas valide" else
      Ok  (tourne_config (fait_dep_unit config c1 c2))

   |(Du(c1,c2))->if est_dep_unit config c1 c2=false  then Error "ce n'est pas valide" else
      Ok (tourne_config (fait_dep_unit config c1 c2)) ;; *)

(* mis_a_jour_configuration ([(6,-3,-3),Rouge;(-4,3,1),Vert],([Vert;Bleu;Jaune;Rouge])) (Du((-4,3,1),(-3,3,0)));;  *)
...
let diff_case (c1:case) (c2:case) = 
  let (i,j,k)=c1 in let (a,b,c)=c2 in 
  ((i-a,j-b,k-c):case) ;;
add_case(1, -2, 1) (-2, -2, 4)  ;;
diff_case  (-3, 0, 3) (-1, 1, 0)  ;;
(* 16 *)
let cases_alignees (c1:case) (c2:case) = 
  let (i,j,k)=c1 in let (a,b,c)=c2 in 
  i =a || j=b || k=c ;;
let case_pivot (c:case) = 
  let (i,j,k)=c in
  ((i/2,j/2,k/2):case);;
...
let rec est_libre_seg_aux c1 config vec  = 
  ...
  if d=1  then true   
    ...
    if quelle_couleur suiv config = Libre then est_libre_seg_aux  suiv  config (v,d-1)
    else false ;;
...
(* let rec est_libre_seg_aux c1 config vec test = 
   let (v,d) =vec in
   if d=1  || test =false then test  
   else 
    if quelle_couleur (diff_case c1 v) config = Libre then est_libre_seg_aux (diff_case c1 v)  config (v,d-1) true
   else est_libre_seg_aux (diff_case (diff_case c1 v) v) config (v,d-1) false ;;
   let est_libre_seg c1 c2 config = 
   let vec = vec_et_dist c1 c2 in 
   if est_libre_seg_aux c1 config vec true then true 
   else false ;; *)
(* est_libre_seg (0, -3, 3) (0, 3, -3) ([ (6,-3,-3),Rouge;(-4,3,1),Vert] 
...
let est_saut c1 c2 config = 
  if quelle_couleur c2 config !=Libre  then false
    ...
    match  calcul_pivot c1 c2  with 
      None -> false 
    | Some c -> 
      if quelle_couleur c config = Libre then false 
      else
      if  est_libre_seg c1 c config  && est_libre_seg c c2 config then true 
      else false ;;

est_saut (-4, 1, 3) (-2, 1, 1) ([(-3, 1, 2),Vert] 
                               ,([Vert;Bleu;Jaune;Rouge])) ;;
(* 20 *)
...
let est_saut_multiple list_case config = 
  ...
  |t::q -> let col =(quelle_couleur (List.hd (list_case)) config)in 
    if  col=t && est_dans_losange (last list_case) 
        && test list_case config then true 
    else false;;
...
let mis_a_jour_configuration config cp = 
  ...
  | (Sm([]))-> Error "Selctioner au moins deux cases"
  |(Sm([c1]))->Error "Selctioner au moins deux cases"
  |(Sm(c1::q))->if  est_saut_multiple (c1::q) config   then 
      let novelle_config = tourne_config (fait_dep_unit config c1 (last (c1::q))) in
      Ok novelle_config
    else Error "ce n'est pas un déplacement multiple" 
;;
(* mis_a_jour_configuration ([(-5, 2, 3),Vert;
   (-2, -1, 3),Rouge; (1, -3, 2),Bleu; (1, -1, 0),Jaune],([Vert;Bleu;Jaune;Rouge]))
   (Sm[(-5, 2, 3); (1, -4, 3); (1, -2, 1); (1, 0, -1)]);; *)
(* 22 *)
let rec dist list= 
  match list with 
    []-> 0 
...
(* est_partie ([(-4, 1, 3),Vert;(-3, 3, 0),Rouge;(-1, 2, -1),Jaune;(-4, 3, 1),Vert]
    ,([Vert;Bleu;Jaune;Rouge])) [Du ((-4, 1, 3),(-3, 1, 2));Sm[(-4, 3, 1); (-2, 3, -1); (0, 1, -1)]] ;; *)
(* 24 *)
let coup_possible config case = [((case:case),((Du(case,case)):coup))];;
(* 25 *)
