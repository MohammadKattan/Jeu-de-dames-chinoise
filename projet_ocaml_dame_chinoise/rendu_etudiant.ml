...
let mis_a_jour_configuration config cp = 
  ...
  | (Sm([]))-> Error "Selctioner au moins deux cases"
  |(Sm([c1]))->Error "Selctioner au moins deux cases"
  |(Sm(c1::q))->if  est_saut_multiple (c1::q) config = false   then Error "ce n'est pas un déplacement multiple" else
...
let rec test_gagnant (config:configuration) = 
  ...
  match col_list with 
    []-> Libre 
  |[c]-> if gagne (case_col_list,[c]) then c else Libre
  |t::q ->  test_gagnant (case_col_list,q);;
...
let rec coup_final list_case_coup ((case_r:case),(coup_r:coup)) = (*4 pour recupérer la distance la plus petite *)
